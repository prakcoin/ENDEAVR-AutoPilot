...
def generate_annotation_prompt(hlc, speed, light, steer, brake, throttle, at_junction, correct_steer=None, correct_brake=None, correct_throttle=None):  
    road_option_dict = {
    ...
    junction_info = "The ego vehicle is at a junction." if at_junction else "The ego vehicle is not at a junction."

    prompt = (
        "Analyze the following sensor data from a front RGB camera in the CARLA Simulator along with additional context data. "
        "The control signals given below were generated by a convolutional neural network.\n\n"
        f"- Current high-level command: {lang_hlc}\n"
        f"- Current speed: {speed:.3f} km/h\n"
        f"- {lang_light}\n"
        f"- {junction_info}\n\n"
        "The predicted control signals generated by the convolutional neural network are as follows:\n\n"
        ...
        f"- Brake value: {brake:.3f} (range: 0.0 to 1.0, where 0.0 indicates no braking (no deceleration), and 1.0 indicates full braking (vehicle comes to a stop))\n"
        f"- Throttle value: {throttle:.3f} (range: 0.0 to 1.0, where 0.0 indicates no acceleration (vehicle not moving), and 1.0 indicates full acceleration (vehicle at maximum speed))\n\n"
    )

    if correct_steer is not None and correct_brake is not None and correct_throttle is not None:
        prompt += (
            "The provided control signals are incorrect. The correct control signals for this situation are as follows:\n\n"
            ...
            "Your task is to assess the original control signals and explain why they are inappropriate for the given situation. "
            "Once you've assessed the issue with the provided signals, justify why the correct control signals are more suitable for the current scenario. "
            "Avoid list format and ensure your reasoning is directly tied to the data presented."
        )
    else:
        prompt += (
            "The provided control signals are correct. Your task is to explain why the provided control signals are appropriate, based on the sensor data and context. "
            "Avoid list format and ensure your reasoning is directly tied to the data presented."
        )

    return prompt

def generate_prompt(hlc, speed, light, steer, brake, throttle, at_junction):  
    road_option_dict = {
        "LaneFollow": "Follow the lane",
        "Left": "Turn left at the junction",
        "Right": "Turn right at the junction",
    ...
    junction_info = "The ego vehicle is at a junction" if at_junction else "The ego vehicle isn't at a junction"
    ...
    prompt = (
        ...
        f"- {lang_light}\n"
        f"- {junction_info}\n\n"
        "The predicted control signals generated from the convolutional neural network are as follows:\n\n"
        f"- Steering angle: {steer:.3f} (range: -1.0 to 1.0, where positive values indicate a right turn and negative values indicate a left turn)\n"
...
def run_episode(world, ego_vehicle, agent, rgb_cam, end_point, collect_correct, episode, args):
    ...
    while True:
        ...
        finetune_prompt = generate_prompt(hlc, speed_km_h, light, selected_control.steer, selected_control.brake, selected_control.throttle, at_junction)
        label = generate_label(correct_control.steer, correct_control.brake, correct_control.throttle)
        annotation_prompt = generate_annotation_prompt(hlc, speed_km_h, light, correct_control.steer, correct_control.brake, correct_control.throttle, at_junction) if collect_correct else generate_annotation_prompt(hlc, speed_km_h, light, incorrect_control.steer, incorrect_control.brake, incorrect_control.throttle, at_junction, correct_control.steer, correct_control.brake, correct_control.throttle)
        ...
        data.append({
            ...
            "annotation_image": f"annotation_images/{image_filename}",
