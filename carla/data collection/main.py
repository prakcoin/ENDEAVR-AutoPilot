...
import queue
import os
from utils import init_world, init_dirs_csv, set_red_light_time, spawn_ego_vehicle, queue_callback, start_camera, create_route
...
# Create image and control queues
image_queue = queue.Queue()
control_queue = queue.Queue()

run_dir, writer, csv_file  = init_dirs_csv(args.town, args.weather)

# Create ego-vehicle and RGB sensor
ego_vehicle = spawn_ego_vehicle(world, spawn_point)
camera = start_camera(world, ego_vehicle, carla.Transform(carla.Location(2,0,1)), queue_callback, image_queue, control_queue)
